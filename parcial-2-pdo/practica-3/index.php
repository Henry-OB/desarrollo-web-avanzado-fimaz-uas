...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($nombre === "" || $apellido === "" || $correo === "") {
        ...
        try {
            ...
            $idAlumno = (int)$pdo->lastInsertId(); // Obtener el ID generado

            // 3) Insertar log (bitacora)
            // Si se marca simular error, forzamos una falta controlada
            if ($simularError) {
                // Forzamos rollback con una exepcion intencional
                throw new Exception("Simulación de error activada: se fuerza rollback.");
                // Alternativa: usar una tabla inexistente para fallar
                //$sqlLog = "INSER INTO logd_alumnos_X (idAlumno,accion) VALUES (:idAlumno, accion)";
            } else {
...
<div class="card">
    <form method="POST">
        <div class="row">
            <div>
                <label>Nombre</label>
                <input type="text" name="nombre" maxlength="15" value="<?=  htmlspecialchars($_POST['nombre'] ?? 'Henry') ?>">
            </div>
            <div>
                <label>Apellido</label>
                <input type="text" name="apellido" maxlength="10" value="<?=  htmlspecialchars($_POST['apellido'] ?? 'Osuna') ?>">
            </div>
            <div>
                <label>Correo</label>
                <input type="email" name="correo" maxlength="50" value="<?=  htmlspecialchars($_POST['correo'] ?? 'Hosuna2345@gmail.com') ?>">
            </div>
        </div>

        <p>
            <label style="font-weight:normal">
                <input type="checkbox" name="simular_error" <?= isset($POST['simular_error']) ? 'checked' : '' ?>>
